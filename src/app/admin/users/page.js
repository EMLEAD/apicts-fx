...
export default function UserManagement() {
  ...
  const [updatingDocStatus, setUpdatingDocStatus] = useState({});
  ...
  const fetchUserDocuments = async (userId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/admin/user-documents?userId=${userId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const updateDocumentStatus = async (documentId, status, rejectionReason = null) => {
    setUpdatingDocStatus(prev => ({ ...prev, [documentId]: true }));
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/admin/user-documents/${documentId}`, {
        method: 'PATCH',
        headers: {
      ...
      setUpdatingDocStatus(prev => ({ ...prev, [documentId]: false }));
    }
  };
              ...
              {currentAdminRole === 'super_admin' && (
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  ...
                  {loadingDocuments ? (
                        ...
                        <div key={doc.id} className="border border-gray-200 rounded-lg p-4">
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              ...
                              <span className={`inline-block mt-1 px-2 py-1 rounded-full text-xs font-medium ${
                                ...
                                doc.verificationStatus === 'expired' ? 'bg-gray-100 text-gray-700' :
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {doc.documentImageFront && (
                              <div>
                                ...
                                <a href={doc.documentImageFront} target="_blank" rel="noopener noreferrer">
                                  <img 
                                    src={doc.documentImageFront} 
                                    alt="Front" 
                                    className="w-full h-32 object-cover rounded border hover:border-blue-500 cursor-pointer"
                                  />
                                </a>
                              </div>
                            )}
                            {doc.documentImageBack && (
                              <div>
                                ...
                                <a href={doc.documentImageBack} target="_blank" rel="noopener noreferrer">
                                  <img 
                          ...
                          {doc.rejectionReason && (
                            <p className="text-sm text-red-600 mt-2">
                              <span className="font-medium">Rejection:</span> {doc.rejectionReason}
                            </p>
                          )}
                            ...
                            <div className="mt-4 pt-4 border-t border-gray-200">
                              <div className="flex flex-wrap gap-2">
                                <button
                                  ...
                                  className="flex items-center space-x-1 px-3 py-1.5 bg-green-600 text-white rounded text-sm hover:bg-green-700 transition-colors disabled:opacity-50"
                                >
                                  <CheckCircle className="h-4 w-4" />
                                  <span>{updatingDocStatus[doc.id] ? 'Verifying...' : 'Verify'}</span>
                                </button>
                                <button
                                  onClick={() => {
                                    const reason = rejectionReasons[doc.id] || prompt('Enter rejection reason:');
                                    if (reason) {
                                      updateDocumentStatus(doc.id, 'rejected', reason);
                                    }
                                  }}
                                  disabled={updatingDocStatus[doc.id]}
                                  className="flex items-center space-x-1 px-3 py-1.5 bg-red-600 text-white rounded text-sm hover:bg-red-700 transition-colors disabled:opacity-50"
                                >
                                  <XCircle className="h-4 w-4" />
                                  <span>{updatingDocStatus[doc.id] ? 'Rejecting...' : 'Reject'}</span>
                                </button>
                              </div>
                            </div>
                          )}
                          
                          {/* Re-verify option for rejected/expired documents */}
                          {(doc.verificationStatus === 'rejected' || doc.verificationStatus === 'expired') && (
                            <div className="mt-4 pt-4 border-t border-gray-200">
                              ...
                              >
                                <CheckCircle className="h-4 w-4" />
                                <span>{updatingDocStatus[doc.id] ? 'Verifying...' : 'Verify Document'}</span>
                              </button>
                            </div>
                          )}
                        </div>
                    ...
                    <p className="text-gray-500 text-center py-8">No documents submitted yet</p>
                  )}
                </div>
